  ...
  <v-row justify="center">
    ...
    >
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="primary"   class="text-right" v-bind="attrs" v-on="on">
          Agregar
        </v-btn>
      </template>
      <v-card>
        <v-card-title>
          <span class="text-h5">Nodo Coordinador</span>
        </v-card-title>
        <v-card-text>
  ...
  export default {
    ...
    methods: {
      ...
      addEquipo(){
        ...
        console.log(data)
        axios
        .post('http://localhost:80/equipos/add_coordinador',data)
